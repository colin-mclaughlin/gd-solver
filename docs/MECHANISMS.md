...
| The motion model is usable for `(y, vy)` pruning | full suite census, 1,168,629 transitions, 51,255 keys | **NOT YET.** `vy' = f(mode,size,speed,upsideDown,onGround,prevHold,hold,vy)` is not a function with that key: **53.65% of keys have nonzero spread, max spread 31.0**, and the bad rate RISES with sample count (0% at 1 sample, 81.5% at 5-19, 82.8% at 20-99) so it is systematic. Ship is worst at 70.4%. The `camila314/gdp` decompilation names the missing axes: the ship's acceleration coefficient is selected by `jumpBuffered`, `isAccelerating` and a "fallingBugged" condition. Geode exposes `m_isAccelerating` (PlayerObject.hpp:2228) and `m_jumpBuffered` (:2241). **A hand-measured custom level would not have revealed this** - it would have produced clean numbers from one combination of axes nobody knew existed. |
