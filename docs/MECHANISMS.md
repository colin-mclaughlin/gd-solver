...
| **The portal roof closes the board** | full suite, `geomPortalRoof = 2`, reproduced bit-for-bit three times including a no-keys default run | **15/15 - every level, first time in the project.** Clubstep 5,930. On the 14 that already solved: **18,626 -> 13,404 deaths, 28% fewer** (Theory of Everything 4215->1324, Electrodynamix 4261->2263). Worse on three (Stereo Madness 216->350, Back on Track 255->288, Electroman 1938->2024). Safety property measured: `roofMargin` min is **0 on every level, never negative**, so the roof never crops below the ceiling the game enforces. Mean margin fell from ~700 to 0-62. |
| The roof's earlier 11/14 was a real verdict | re-measured after `geoStateRestore` | **No - it was a correct mechanism judged by a broken instrument.** The 11/14 was taken on the build whose search tree was history-dependent. The ordering had the roof programme CUT. Fixing the tree is what made it measurable, which is the return on committing that fix for correctness rather than score. |
| 4b's poor showing was caused by the phantom ceiling | `V` re-tested on the corrected roof | **No - predicted and refuted.** Same signature as before: xStep 2153->1065 and Electroman 2024->1665, but Theory of Everything 1324->3772 and **Clubstep FAILS**. 14/15, and +5% deaths on the 14 common levels. The roof had nothing to do with it. `V` stays off, now on a correct map. |
| `prevAirMode` / `prevOnGround` were not restored on a reposition | audit, then measured | **Real, and fixed.** Both gate FORCED decisions - at a mode transition and at a landing - and neither was put back on a rewind, so the first step after one compared against the abandoned path. Exposure measured at **63-212 repositions per level (2.8%-54%)**. Fixed via the `enteringHold` idiom across all four restore paths. Result: **15/15 held, Clubstep 5,930 -> 5,558, all other levels bit-identical.** The `prevOnGround` half is inert either way; `prevAirMode` is the whole effect. |
| The `enteringHold` idiom can be applied by pattern | `prevOnGround` | **No.** `prevAirMode` is written only inside the transition branch, so it always equals the mode at the current step by push time - capture correct. `prevOnGround` is assigned AFTER the push, so capturing it gave the previous step's value. The off-by-one **survived a full suite run** because the correct and incorrect values produce byte-identical boards. Fixed by capturing `m_player1->m_isOnGround` directly. **Check the field's update ordering; do not pattern-match.** |
| `modeStale` can verify the fix | intended as a control | **No.** It compares stale against recorded at reposition time, so it measures EXPOSURE, not residual, and does not drop when the fix works. There is still no direct measurement that spurious forced decisions are gone - only sound reasoning and a holding board. A non-circular check would count forced transitions landing within one step of a reposition. |
| `ceilMaxSeen` / `floorMinSeen` need restoring too | instrumented `usable()` | **No - deleted instead.** The phantom-space filter they fed rejected **0 of 126,140** intervals across all fifteen levels: `geomPortalRoof` now removes phantom space at map-build time, so the filter is redundant. Removing it eliminated two run-wide history-dependent accumulators **and** a dead branch. Verified behaviour-neutral: the suite is bit-identical after deletion. Third instance of the defect class, resolved by removal rather than by machinery. |
